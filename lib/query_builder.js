...
var op_parser = require('./operator_parser')
...
var buildQueryFromExpression = function (entp, query_parameters, values) {
  ...
  function parseComplexSelectOperator (current_name, current_value, params) {
    for (var op in current_value) {
      ...
      var err = op_parser[op](current_name, op_val, params, values)
      if (err) {
        return {err: err}
      }
    }
...
function savestm (ent) {
  ...
  stm.text = 'INSERT INTO ' + table + ' (' + escapedFields + ') values (' + relationalstore.escapeStr(params) + ')'
  stm.values = values
...
function updatestm (ent) {
  ...
  stm.text = 'UPDATE ' + table + ' SET ' + params + " WHERE id='" + relationalstore.escapeStr(ent.id) + "'"
  stm.values = values
...
function filterStatement (qent, q) {
  ...
  stm.text = select + relationalstore.escapeStr(selectColumns.join(',')) + ' FROM ' + table + wherestr + relationalstore.escapeStr(metastr)
  stm.values = values
...
function selectstm (qent, q, done) {
  ...
  stm.text = 'SELECT ' + what + ' FROM ' + table + (wherestr ? ' WHERE ' + wherestr : '') + relationalstore.escapeStr(metastr)
  stm.values = values
...
function selectstmOr (qent, q) {
  ...
  stm.text = 'SELECT * FROM ' + table + wherestr + relationalstore.escapeStr(metastr)
  stm.values = values
